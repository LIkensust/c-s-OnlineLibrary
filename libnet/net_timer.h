...
public:
  NetTimer(const Timestamp &when, const TimeoutCallBack &tcb)
      : _expireTime(when), _callBack(tcb), _delete(false) {}

  void setDel() { _delete = true; }

  bool isDeleted() { return _delete; }

  Timestamp getExpireTime() const { return _expireTime; }

  void runCallBackFunc() { _callBack(); }
...
private:
  using TimerQueue =
      std::priority_queue<NetTimer *, std::vector<NetTimer *>, cmp>;
...
public:
  ...
  void updateTime() { _now = Clock::now(); }

  void addTimer(NetHTTPRequest *req, const int &timeout,
                const TimeoutCallBack &tcb) {
  ...
  void handleExpireTimers() {
    ...
    while (!_timeQueue.empty()) {
      ...
      // 优先级队列头没有超时
      if (std::chrono::duration_cast<MS>(timer->getExpireTime() - _now)
              .count() > 0) {
        return;
      }
  ...
  int getNextExpireTime() {
    ...
    while (!_timeQueue.empty()) {
      ...
      res =
          std::chrono::duration_cast<MS>(timer->getExpireTime() - _now).count();
      res = (res < 0) ? 0 : res;
      break;
    }
