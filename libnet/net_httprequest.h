...
public:
  NetHTTPRequest(int fd)
      : mFd(fd), mWorking(false), mCgi(false), mTimer(nullptr),
        mState(ExpectRequestLine), mMethod(INVALID), mVersion(UnKnown) {
    assert(fd >= 0);
    PLOG(DEBUG, "create the request");
  }

  ~NetHTTPRequest() { ::close(mFd); }

  int getSockFd() { return mFd; }
  ...
  // 定时器相关
  void setTimer(NetTimer *timer) { mTimer = timer; }

  NetTimer *getTimer() { return mTimer; }

  void setWorking() { mWorking = true; }

  void setNoWorking() { mWorking = false; }

  bool isWorking() const { return mWorking; }

  // cgi
  bool isCGi() { return mCgi; }
  ...
  bool parseFinish() { return mState == GotAll; }
...
private:
  ...
  // 设置post参数
  void __setQuery(const char *begin, size_t len) { mQuery.assign(begin, len); }
  // 设置版本
  void __setVersion(gHttpVersion ver) { mVersion = ver; }
  ...
  std::string __getContenLength() { return mHeaders["Content-Length"]; }

  gHttpMethod __getMethod() { return mMethod; }
