...
void free_part_of_map() {
  ...
  if (size > 200000) {
    ...
    do {
      cmp tmp;
      if (cache.size() <
          200000) //防止重复释放 因为在进行下一步之前可能别的线程已经释放过一次
      {
        break;
      } else {
...
void main_job(int fd, shared_ptr<char> &buf) {
  ...
  if (msg.type == READ) {
    ...
    if (flag == true) {
...
#endif
        void *fp = mmap(NULL, (size_t)filesize, PROT_READ, MAP_SHARED, file_fd,
                        (off_t)0);
        close(file_fd);
#ifdef TEST
        cout << "fp:" << fp << endl;
      ...
      it->second.per =
          (3 * it->second.per + 7 * (it->second.lasttime - tmp)) / 10;
      pthread_rwlock_unlock(&map_lock);
    }
    //在缓存区内
    pthread_rwlock_rdlock(&map_lock);
    ...
    if (flag == true) {
      ...
      if (stat(ret.c_str(), &statbuf) < 0) {
        ...
        tmp.data = (char *)mmap(NULL, stbuf.st_size, PROT_READ, MAP_SHARED,
                                tmpfd, (off_t)0);
#ifdef TEST
        cout << "new mmap:";
        printf("%s\n", tmp.data);
...
void Do_read_wirte(int fd) {
  ...
  pthread_create(&p_t, &att, FUNPTR(pthread_prudect),
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < cpu_num; i++) {
    pthread_create(&pthread_id[i], &att, (FUNPTR)pthread_handler,
                   reinterpret_cast<void *>(id[i]));
  }
  delete[] id;
  int i;
  while (1) {
    memset(event_buf, 0, sizeof(epoll_event) * EVENTSIZE);
    int size = epoll_wait(ep_fd, event_buf,
                          sizeof(struct epoll_event) * EVENTSIZE, -1);
    if (size < 0) {
      perror("epoll_wait");
      return 0;
    }
